"""Rejected/unfinished image-region job verification cannot retain the preceding success projection."""
import json
import datetime
...
import test_image_region_jobs as runner
...
class EvidenceTests(unittest.TestCase):
    def check_failure(self, phase, failure, action):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                latest = json.loads((root / "artifacts/image-region-jobs-result.json").read_text())
                self.assertFalse(latest["passed"])
                self.assertEqual(latest["phase"], phase)
                self.assertEqual(latest["failure"], failure)
                self.assertEqual(len(list((root / "artifacts/image-region-jobs").glob("*.json"))), 2)
                self.assertNotIn("private-path", json.dumps(latest))

    def test_inventory_rejection_replaces_prior_success(self):
        rejection = patch.object(runner.image_runner, "inspect_runtime",
                                 side_effect=runner.image_runner.VerificationFailure("unexpected_image_classpath"))
        self.check_failure("runtime_inventory", "unexpected_image_classpath", rejection)
    ...
    def test_timeout_replaces_prior_success(self):
        ...
        with patch.object(runner.image_runner, "inspect_runtime", return_value={}):
