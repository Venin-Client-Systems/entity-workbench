"""Rejected/unfinished OCR word-region verification cannot retain the preceding success projection."""
import json
import datetime
...
import test_ocr_regions as runner
...
class EvidenceTests(unittest.TestCase):
    def check_failure(self, phase, failure, action):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "fixtures/ocr").mkdir(parents=True)
            (root / "fixtures/ocr/synthetic.pgm").write_bytes(b"synthetic fixture")
            with (
                ...
                latest = json.loads((root / "artifacts/ocr-regions-result.json").read_text())
                self.assertFalse(latest["passed"])
                self.assertEqual(latest["phase"], phase)
                self.assertEqual(latest["failure"], failure)
                self.assertEqual(len(list((root / "artifacts/ocr-regions").glob("*.json"))), 2)
                self.assertNotIn("private-path", json.dumps(latest))

    def test_inventory_rejection_replaces_prior_success(self):
        rejection = patch.object(runner.ocr_runner, "inspect_runtime",
                                 side_effect=runner.ocr_runner.VerificationFailure("incomplete_runtime_inventory"))
        self.check_failure("runtime_inventory", "incomplete_runtime_inventory", rejection)
    ...
    def test_timeout_replaces_prior_success(self):
        ...
        with patch.object(runner.ocr_runner, "inspect_runtime", return_value={}):
    ...
    def test_compiler_failure_replaces_prior_success(self):
        def failed(_runtime, report):
            report["phase"] = "compile_probe"
            raise subprocess.CalledProcessError(1, ["private-path"], stderr="private-path")
        with patch.object(runner.ocr_runner, "inspect_runtime", return_value={}):
            action = patch.object(runner, "native_tests", side_effect=failed)
            self.check_failure("compile_probe", "tool_failed", action)
