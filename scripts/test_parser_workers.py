...
SOURCES = ["crates/core/src/engines.rs", "crates/core/src/engines/parser.rs", "crates/core/src/engines/parser/tests.rs", "crates/core/src/engines/supervision.rs", "crates/core/src/engines/supervision/tests.rs", "workers/java/src/main/java/workbench/ParseWorker.java", "workers/java/src/main/java/workbench/SearchWorker.java", "workers/java/src/main/java/workbench/Protocol.java", "workers/java/src/main/java/workbench/HostileProbe.java", "scripts/test_parser_workers.py"]
EXPECTED = {"cancelled_parser_never_launches_or_stages_input", "parser_results_require_source_binding_and_honest_status", "native_parser_extracts_supported_formats_and_preserves_limitations", "native_parser_has_no_search_access_and_cancels_running_worker", "index_acknowledgement_is_required_before_revision_publication", "cleanup_repairs_directories_without_touching_link_targets_and_reports_failure", "index_rejects_links_and_tree_budget_overruns", "native_java_hostile_and_benign_boundaries", "native_lucene_uses_separate_jobs_and_read_only_search_index", "process_setup_closes_inheritable_descriptor_and_reaps_timeout_group", "profile_has_distinct_read_and_write_access", "result_rejects_links_and_oversize_and_special_files"}
...
def main():
    ...
    command = ["cargo", "test", "--locked", "-p", "workbench-core", "--lib", "engines::", "--", "--include-ignored", "--test-threads=1", "--nocapture"]
