"""Rejected/unfinished PDF verification cannot retain the preceding success projection."""
import json
import datetime
...
import test_pdf_workers as runner
    ...
    def check_failure(self, phase, failure, action):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                latest = json.loads((root / "artifacts/pdf-render-result.json").read_text())
                self.assertFalse(latest["passed"])
                self.assertEqual(latest["phase"], phase)
                self.assertEqual(latest["failure"], failure)
                self.assertEqual(len(list((root / "artifacts/pdf-render").glob("*.json"))), 2)
                self.assertNotIn("private-path", json.dumps(latest))

    def test_inventory_rejection_replaces_prior_success(self):
        rejection = patch.object(runner, "inspect_runtime",
                                 side_effect=runner.VerificationFailure("unexpected_pdf_classpath"))
