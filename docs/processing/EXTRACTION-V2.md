...
The [native compatibility campaign](verification/extraction-v2-native-mac.json) passed all fourteen parser/search boundary cases on the signed, clean implementation revision on an Apple Silicon Mac. It used the existing macOS parser and separate search recipes, including synthetic hostile-worker, cancellation and isolation probes. The new local-font semantic check in that campaign is a synthetic shared-validator test; actual Windows execution of that recipe remains unverified here.
