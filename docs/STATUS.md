...
| Identity | Typed entity model, namespaces and leading zeros, supporting/conflicting observations in demo, merges and reversals | General entity/observation authoring; calibrated Splink linkage; full identity comparison workflow |
| Local discovery | Selected-host direct HTTPS collection, robots rules, DNS pinning, redirect checks, request/hop/time bounds, source retention | Durable frontier/resume, cancellation, source-specific adapters, browser capture, full broad-web coverage |
...
| Distribution | macOS Apple Silicon development binary/app build with Java/Lucene staging | Complete runtime inventory on all targets; Windows Fixed WebView2; offline clean-machine tests; signing/notarization |
