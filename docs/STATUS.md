...
|---|---|---|
| Product design | Native editable Figma frames, industrial transaction revision and statement mapping/preview; graphite/amber palette, joined instruments, square controls, bundled Inter/JetBrains Mono; desktop panel and compact dialog; exported comparisons, axe and keyboard verification | Industrial compact/remaining workflow frames, long-content refinement, icon review, manual accessibility and final visual sign-off |
| Workspace | Rust-owned SQLite, revision history, content-addressed originals, strict commands, recoverable backup/restore, transactional v1→v2 upgrade with failure injection and evidence recovery, newer-schema refusal | Further migrations and interrupted-upgrade coverage; full Windows ACL implementation; workspace management UI |
