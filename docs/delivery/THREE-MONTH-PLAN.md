...
The calendar assumes concurrent platform/distribution, evidence/analysis, and analyst-workflow/design effort, plus timely maintainer reviews and native test access. A single serial implementation stream is unlikely to deliver the entire agreed scope in this window. Parallel-agent execution is a separate owner preference; the schedule itself does not authorize delegation, and current work can continue in one stream while that preference is unresolved. No unattended three-month scheduler is installed by this plan. Execution proceeds while the agent is active, and each continuation resumes from the issue board and recorded evidence.
...
All twelve machine-readable gates in `docs/release-gates.json` are mapped in `gate-coverage.json`; issue closure alone does not pass a gate. EW-02 will require evidence records rather than naked booleans. Cross-platform gates require real evidence on every claimed target. The release is complete only after EW-40 reconciles the gate evidence with the exact downloadable signed binaries.
