...
- Prepared native execution: fourteen Windows unit tests, 17 permission observations with an unconfined control, ten hostile/resource/cleanup modes and copied Java 21 assigned-I/O compatibility. Native test results are recorded separately below; the full confined harness and Java results remain **unverified**.
...
The earlier explicit environment contained only `SystemRoot`, `WINDIR`, `TEMP` and `TMP`. The revised environment supplies the strict OS-derived profile allowlist documented above. This is a candidate repair for the environment-related launch failure; the precise missing variable and successful native launch are not established by local cross-compilation. Four additional Windows tests check the exact allowlist and ordering, rejection of malformed required values, lossless UTF-16 handling and construction from the real OS environment. Native execution of this correction, the confined permission/resource modes and Java compatibility are pending. No fallback or permission expectation was weakened.
