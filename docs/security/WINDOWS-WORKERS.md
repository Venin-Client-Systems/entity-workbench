...
- `cargo test -p workbench-windows-worker --locked`: five macOS-hosted tests passed.
...
- Prepared native execution: sixteen Windows unit tests, 18 permission/console observations with an unconfined control, ten hostile/resource/cleanup modes and copied Java 21 assigned-I/O compatibility. Native test results are recorded separately below; the full confined harness and Java results remain **unverified**.
...
The synthetic worker now writes fixed enum checkpoints immediately before token operations, console query, each permission probe, inherited-handle seek/read, network tests, child spawn and result write. Its baseline must reach `completed`. The opt-in `run_probe` wrapper runs the identical policy and reads at most 64 bytes of one ordinary, single-link checkpoint file after observing process exit and before normal cleanup. Only recognized enum values enter the report; paths, arbitrary diagnostic strings and document data are rejected. These are untrusted diagnostic hints, never acceptance evidence. A missing usable checkpoint cannot by itself prove that execution stopped before `main`. Ordinary adapter `run` calls collect no checkpoint, and termination/cleanup failures continue to reject results. The next native run must locate the invalid-handle failure; assertions and permissions remain unchanged.
