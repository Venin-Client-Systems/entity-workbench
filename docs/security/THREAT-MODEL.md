...
| Worker → workspace | No worker database path; versioned messages; selected output/IDs/revision validated | Windows application-worker integration/installed testing outstanding; macOS signed helpers absent |
...
| Hostile parsing | Separate disposable Mac parser/image/PDF/OCR workers, strict output validation, fixed recipes and immutable unreviewed derivatives | Scan-focused formats and English only; signed helpers, full format/decoder resource campaigns, hard RSS/disk bounds and supervisor-crash recovery outstanding |
...
| Statistical identity | No automatic merge; statistical linkage disabled without calibrated model | Calibration, bias/error evaluation, advanced comparison and independent-source modelling |
...
The available Mac now has retained actual document/image/PDF/OCR recognition and hostile-worker evidence, including explicit process reaping, fixed output paths and schema/hash/geometry validation. The [verification record](../VERIFICATION.md) binds each observation to source, runtime and synthetic inputs. Word-region confidence is an uncalibrated engine score; paired text/TSV agreement cannot establish correctness or completeness. No extraction becomes an accepted fact automatically. These development observations do not replace the release gates or independent review.
