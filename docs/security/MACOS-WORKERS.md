...
Each request receives a new private job directory, explicitly removed and checked after success or failure. Cleanup repairs owner permissions only on ordinary directories using no-follow operations, never on files or links. A cleanup error rejects the result and retains the original worker error in the reported failure; destructor cleanup is not treated as successful erasure. Rust retains the canonical workspace and originals. Removal uses the standard library’s symlink-safe directory deletion after permission repair. [Rust remove_dir_all](https://doc.rust-lang.org/std/fs/fn.remove_dir_all.html). A coordinator-owned lock serializes access to the local index. A failed rebuild removes the revision marker so the next attempt cannot mistake partial output for a current index. Rust validates an exact index acknowledgement against the assigned revision and expected document count before publishing the marker; malformed, extra, trailing or inconsistent output is rejected. Each rebuild discards its old derivative without following links. An invalid current index also triggers this rebuild path, so failed writes cannot permanently poison retries.
...
On 24 September 2026, the seven supervisor tests passed on an **arm64 development Mac running macOS 26.6.2**, using **Java 21.0.12.1**, **Lucene 10.5.1** and the locally rebuilt current adapter. No runtime download was required. The evidence report is retained locally with the exact source/runtime hashes. Full Rust regression tests and strict Clippy also passed; see the PR validation for counts at the reviewed revision.
