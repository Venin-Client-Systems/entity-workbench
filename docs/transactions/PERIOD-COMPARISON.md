...
This bounded EW-18 slice implements a read-only Rust calculation for two explicitly selected periods. It supplies a typed domain result and `Workspace::compare_transaction_periods(request, expected_revision)`. Command dispatch, published schema files, an editable design frame and a UI are deliberately left for the next integration step. It does not complete account-flow graphs, statement coverage, reconciliation, reporting integration or performance acceptance.
...
Local verification on this slice: `cargo test -p workbench-core` passed 127 ordinary tests, including 10 new comparison tests and 11 existing pattern tests; 11 native-runtime tests were ignored and are not claimed as verified by this run. `cargo clippy -p workbench-core --all-targets -- -D warnings` passed. The staged public audit used the private exclusion list outside the repository. These checks cover the Rust slice only; no browser or release test is claimed.

Before exposing this capability, integrate a versioned command/schema and an editable, reviewed design with real-core UI tests. The UI must show periods, duration differences, observed-versus-missing denominators and transfer treatment; retain exact ratios; and disable stale result drillthrough. Account-flow graphs remain a separate slice. No native packaging, release, performance or complete EW-18 acceptance is claimed here.
