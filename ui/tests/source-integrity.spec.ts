...
test("direct transaction and source excerpts refuse an altered original despite a cached derivative", async ({ page }) => {
  rmSync(root, { recursive: true, force: true });
  const w: Workspace = core({
    action: "import", name: "synthetic.csv",
    bytes: [...Buffer.from("account,date,description,amount,currency\n0042,2025-01-01,Synthetic exact purchase,-0.10000001,AUD\n")],
  }).workspace;
  await page.goto("/");
  ...
  const bytes = readFileSync(original), mode = statSync(original).mode;
  chmodSync(original, 0o600);
  try {
    // Same length proves this is digest verification, not just a size check.
    const changed = Buffer.from(bytes); changed[0] = "X".charCodeAt(0);
    writeFileSync(original, changed);
    await page.getByRole("button", { name: "Synthetic exact purchase", exact: true }).click();
    const excerpt = page.getByRole("region", { name: "Original transaction excerpt" });
    await expect(excerpt.getByRole("alert")).toContainText("Original evidence checksum mismatch");
    ...
    await expect(source.getByRole("alert")).toContainText("Original evidence checksum mismatch");
    await expect(source.locator(".source-quote")).toHaveCount(0);
    await expect(source).not.toContainText("Validated against workspace revision");
    ...
    await expect(source).toContainText(`Validated against workspace revision ${w.revision}`);
