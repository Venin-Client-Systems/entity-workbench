use super::*;
#[test]
fn pdf_profile_adds_only_its_assigned_raster_output() {
    ...
    assert!(!profile.contains("/runtime/image"));
...
#[test]
#[ignore = "requires staged PDF renderer and hostile fixture"]
fn native_pdf_worker_denies_outside_io_network_and_verifies_cancel_and_output_bound() {
    use std::net::TcpListener;
    let runtime = std::path::PathBuf::from(
        std::env::var_os("WORKBENCH_TEST_PDF_RUNTIME").expect("PDF runtime required"),
    );
    ...
    assert!(matches!(timeout, Err(Error::QuotaExhausted(_))));
    let pid: i32 = fs::read_to_string(job.path().join("result.json"))
        .unwrap()
        .parse()
        .unwrap();
    assert_eq!(unsafe { libc::kill(pid, 0) }, -1);
