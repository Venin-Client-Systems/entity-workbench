...
fn schema_one_upgrade_creates_a_recoverable_backup_with_originals() {
    ...
    assert_eq!(upgraded.view().unwrap().schema_version, 5);
...
fn failed_upgrade_rolls_back_version_and_records_and_retains_backup() {
    ...
    let conn = rusqlite::Connection::open(&db).unwrap();
    conn.pragma_update(None, "user_version", 1).unwrap();
    conn.execute_batch("CREATE TRIGGER reject_upgrade BEFORE INSERT ON events WHEN NEW.action='workspace.schema_v5' BEGIN SELECT RAISE(ABORT,'synthetic failure'); END;").unwrap();
    drop(conn);
